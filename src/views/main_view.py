...
class MainView:
    ...
    def __init__(self, title: str = "Dota 2 Auto Accept - Control Panel", config_model=None):
        ...
        self.on_telegram_enabled_change = None
    ...
    def set_telegram_enabled(self, enabled: bool):
        if hasattr(self, 'telegram_enabled_var'):
            self.telegram_enabled_var.set(enabled)

    def set_telegram_bot_token(self, bot_token: str):
        if hasattr(self, 'telegram_bot_token_entry'):
    ...
    def _create_permanent_settings(self, parent=None):
        ...
        # Create the settings sections in tabs
        self._create_tabbed_settings_panel(parent=parent)

    def _create_tabbed_settings_panel(self, parent=None):
        parent = parent or self.window
        ...
        telegram_frame = tk.Frame(notebook, bg="#ffffff")
        general_frame.pack(fill="both", expand=True)
        telegram_frame.pack(fill="both", expand=True)
        notebook.add(general_frame, text="General")
        notebook.add(telegram_frame, text="Telegram")

        self._create_audio_settings(parent=general_frame)
        self._create_monitor_settings(parent=general_frame)
        self._create_telegram_settings(parent=telegram_frame)

    def _create_telegram_settings(self, parent=None):
        parent = parent or self.window
        ...
        self.telegram_enabled_check = tk.Checkbutton(
            telegram_frame,
        ...
        self.telegram_enabled_check.pack(pady=(5, 0), padx=8, anchor="w")
        ...
        tk.Label(
            ...
            bg="#ffffff",
            fg="#333"
        ).pack(pady=(5, 0), anchor="w", padx=8)
        self.telegram_bot_token_entry = ttk.Entry(telegram_frame, font=("Segoe UI", 9))
        self.telegram_bot_token_entry.pack(pady=3, padx=8, fill="x")
        ...
        self.telegram_message_entry = tk.Entry(telegram_frame, font=("Segoe UI", 9))
        self.telegram_message_entry.pack(pady=3, padx=8, fill="x")
        ...
        self.telegram_test_btn = tk.Button(
            telegram_frame,
            text="Test Telegram",
        ...
        self.telegram_test_btn.pack(pady=(10, 6), padx=8, anchor="w")

        self.telegram_info_label = tk.Label(
            telegram_frame,
            text="Bot token será usado para obter o chat ID automaticamente.",
            font=("Segoe UI", 8),
            fg="#555",
            bg="#ffffff"
        )
        self.telegram_info_label.pack(pady=(0, 8), padx=8, anchor="w")

    def _on_telegram_enabled_change_event(self):
        if self.on_telegram_enabled_change:
            self.on_telegram_enabled_change(self.telegram_enabled_var.get())

    def _on_telegram_bot_token_change_event(self, event=None):
        if self.on_telegram_bot_token_change:
            self.on_telegram_bot_token_change(self.telegram_bot_token_entry.get())
