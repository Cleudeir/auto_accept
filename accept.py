...
def show_audio_settings():
    ...
    )  # Now returns list of (display_name, index) or empty
    ...
    if monitors_available:
        ...
        if not found_saved_setting:
            ...
            selected_monitor_capture_setting = monitors_available[0][
                1
            ]  # Update global to the first available
        ...
        selected_monitor_capture_setting = (
            -1
        )  # Indicate no valid monitor can be selected
    ...
    def on_monitor_select(event=None):
        ...
        if monitors_available and 0 <= idx < len(monitors_available):
            ...
            selected_monitor_capture_setting = monitors_available[idx][
                1
            ]  # Get the monitor index (int)
            save_audio_settings()
            logger.info(
    ...
    log_text = tk.Text(log_frame, height=8, width=50, font=("Consolas", 8))
    ...
    # Function to refresh screenshot preview
    def refresh_screenshot():
        ...
        # Schedule the next refresh
        win.after(2000, refresh_screenshot)  # Refresh every 2 seconds

    # Controls for screenshot viewer
    screenshot_controls_frame = tk.Frame(screenshot_frame)
    screenshot_controls_frame.pack(fill="x", pady=(5, 0))

    refresh_screenshot_btn = tk.Button(
        screenshot_controls_frame,
        text="🔄 Refresh",
        command=lambda: refresh_screenshot(),
    )
    refresh_screenshot_btn.pack(side=tk.LEFT, padx=2)

    # Label to display filename
    filename_label = tk.Label(screenshot_controls_frame, font=("Arial", 7), fg="gray")
    filename_label.pack(side=tk.RIGHT, padx=2)

    # Call refresh_screenshot initially to populate the screenshot viewer
    refresh_screenshot()
...
def main_loop():
    ...
    while is_running:
        ...
        screenshot_filename = os.path.join(
            folder, f'dota2_monitor_capture_{time.strftime("%Y%m%d-%H%M%S")}.png'
        )

        if save_dota2_monitor_screenshot(
